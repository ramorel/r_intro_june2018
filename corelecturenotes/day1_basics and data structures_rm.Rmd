...
## Why R? - NEED TO COMPLETE

## RStudio and RStudio Server - NEED TO COMPLETE
...
## Working Directory [NEED TO FIGURE OUT HOW THIS WORKS WITH RSTUDIO SERVER]
...
# Using a local file
gapminder5 <- read.csv("data/gapminder5.csv")
```
...
```{r}
gapminder5 <- read.csv("data/gapminder5", 
...
In the previous section, we imported two datasets. For the rest of today, we will focus on the Gapminder data, which is stored in our environment as `gapminder5`. To refresh yourself, you can view the data frame in R using the `View()` function.
...
Other useful functions are `dim()` which shows the dimensions of the data frame, and `head()` which shows the first few rows of the data frame (5 rows by default).
...
```{r}
table(gapminder5$continent, gapminder5$year)
```
...
We can also use functions like `mean()`, `median()`, `mode()`, `var()`, and `sd()` to calculate other summary statistics for individual variables. For example, let's calculate the mean of life expectancy. Recall that we can use the `$` operator to call up a variable within a data frame using its name.

```{r}
mean(gapminder5$lifeExp)
```
...
## Adding and removing columns - UNFINISHED
...
```{r}
aggregate(gapminder07$lifeExp, by=list(continent=gapminder07$continent), FUN=mean)
```
...
```{r}
aggregate(gapminder5$lifeExp, by=list(year=gapminder5$year, continent=gapminder5$continent), FUN=mean)
```

## Statistics

Now that we have data ... we can do statistics! You will learn more about particular statistical models and methods over the course of your program. For now, let's do some basic things to get a feel for how R handles statistical analysis.
...
Think of `~` as "as a function of". So the above code reads "GDP per capita as a function of life expectancy", meaning grouped by or explained by.
...
Let's try another regression with two indpendent variables. This time, we will use the full `gapminder5` data and predict life expectancy as a function of GDP per capita and year.

```{r}
r2 <- lm(lifeExp ~ gdpPercap + year, data=gapminder5)
summary(r2)
```
...
We imported the `gapminder5` data earlier in CSV format, and manipulated it in several ways: we subsetted the 2007 data and added the variables `lifeExp_rounded`, `lifeExp_highlow`. and `continent_abrv`. 
...
```{r, eval=FALSE}
write.csv(gapminder07, file="data/gapminder5_2007_edited.csv", 
          row.names=FALSE)
```
...
```{r, eval=FALSE}
write_csv(schooldata, "data/gapminder5_2007_edited.csv")
```
